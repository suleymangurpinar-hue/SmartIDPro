...
class AiScanWorkspace extends StatelessWidget {
  ...
  const AiScanWorkspace({super.key, this.imagePath});
          ...
          child: Stack(
            children: [
              ...
              const Positioned.fill(
                child: IgnorePointer(child: IcaoCropOverlay()),
              ),
              ...
              const Positioned(
                left: 24,
                bottom: 24,
                child: _HudTag('FACE DETECTED'),
              ),

              const Positioned(
                right: 24,
                bottom: 24,
                child: _HudTag('ICAO READY'),
              ),
              ...
              if (imagePath != null)
                Positioned.fill(
                  child: IgnorePointer(child: LiveFaceMeshOverlay(mesh: mesh)),
                ),
            ],
          ),
        );
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: const Color(0xFF45A9FF)),
      ),
      child: Text(
        title,
...
class _HudGridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 1; i <= 5; i++) {
      canvas.drawCircle(center, i * 120, radar);
    }
  }
